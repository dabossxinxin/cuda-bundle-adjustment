	...
	*/
	...
	public:
			...
			const EdgeSet2D& edgeSet2D, const EdgeSet3D& edgeSet3D, const CameraParams& camera)
		{
			...
			for (const auto& itVertexP : vertexMapP)
			{
				if (!itVertexP.second->fixed)
				{
					itVertexP.second->iP = numP++;
					verticesP_.push_back(itVertexP.second);
					qs_.emplace_back(itVertexP.second->q.coeffs().data());
					ts_.emplace_back(itVertexP.second->t.data());
				}
				else
				{
					fixedVerticesP_.push_back(itVertexP.second);
				}
			}

			// assign landmark vertex id
			// gather 3D positions into vector
			for (const auto& itVertexL : vertexMapL)
			{
				if (!itVertexL.second->fixed)
				{
					itVertexL.second->iL = numL++;
					verticesL_.push_back(itVertexL.second);
					Xws_.emplace_back(itVertexL.second->Xw.data());
				}
				else
				{
					fixedVerticesL_.push_back(itVertexL.second);
				}
			}
